...
export async function createBlog(blog) {
  return await execAsync("INSERT INTO blogs " +
    "(blog_id, title, brief, text, user_id, username) VALUES (?, ?, ?, ?, ?, ?)",
    [blog.blog_id, blog.title, blog.brief, blog.text, blog.user_id, blog.username],
    "create blog " + JSON.stringify(blog));
}

export async function getOpenBlog(blog_id) {
  return await execAsync("SELECT blog_id, title, brief, text, user_id, username, create_time, last_edit_time, views"
...
export async function getOpenBlogList(user_id, offset, limit) {
  return await execAsync("SELECT blog_id, title, brief, user_id, username, create_time, last_edit_time, views"
    + " FROM blogs WHERE user_id = ? AND is_hidden = false LIMIT ?, ?",
...
export async function getBlogList(user_id, offset, limit) {
  return await execAsync("SELECT blog_id, title, brief, user_id, username, create_time, last_edit_time, views"
    + " FROM blogs WHERE user_id = ? LIMIT ?, ?",
    [user_id, limit || 10, offset || 0],
    `get open blog list by user_id ${user_id}`);
}
